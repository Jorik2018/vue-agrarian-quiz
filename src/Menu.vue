<template>
    ...
    padding-left: 10px;"><div style="color:blue">Hola,<br/>{{o.display_name}}</div></td>
						...
						<span style="right: 0px;position: absolute;top: 10px;" v-on:click="$ionic.menuController.close('menuprincipal')"><i class="fa fa-times"></i></span>
					</ion-title>
			...
			<ion-content>
				...
				<ion-item v-if="session.perms&&session.perms.supervise" href="/admin/supervise">
					<i class="fa fa-users"></i>Monitores
				</ion-item>
				<ion-item class="_">
					<i class="fa fa-exclamation-triangle"></i><v-checkbox v-model="app.connected" v-on:input="vv(app.connected)" label="Connectado"/>
				</ion-item>
