<template>
	<v-form header="FICHAS DE ENCUESTA" style="position:relative;" action="/admin/poll">
		<v-table store="poll" 
v-on:row-select="rss" :filters="filters"
		row-style-class="row.synchronized?'green':(row.tmpId>0?'yellow':'')" src="/api/poll" pagination="50" 
		scrollable="true" v-on:updated="app.bindLinks($el)">
						<template v-slot:header="">
                <v-button value="Crear" icon="fa-plus" 
                    class="on" v-on:click.prevent="create"></v-button>
<v-button value="Grabar" icon="fa-save" v-show="app.connected"
                    class="on" v-on:click.prevent="sync({sufix:'/'+app.poll})"></v-button>
                ...
                <v-button value="Editar" v-if="1||can(perms.DESARROLLO_SOCIAL_REGISTER_SIVICO)" icon="fa-pen" 
					...
					<v-button value="Eliminar" icon="fa-trash" v-show="rowSelectedCount2"
			...
			<template v-slot:default="{row}"> 
				...
				<td header="IDENTIFICACION MUESTRAL" width="300"><a v-bind:href="'/admin/poll/'+(row.tmpId?-row.tmpId:row.id)">URB_RUR={{row.urb_rur}}
				...
				<td header="MIEMBROS" width="140" class="center"> {{row.people?row.people.length:0}}</td>
				<td header="FECHA" width="120" class="center"> {{row.tmpId|date}} </td> 
			</template>
		</v-table>
	</v-form>
</template>
